...
const TARGET_NAMES = [
  "BALANCE_OFFSET_L3_Elite_Standard_Defence",
  "BALANCE_OFFSET_L3_Elite_Heavy_Defence",
  "BALANCE_OFFSET_L3_Elite_Physical_Biased",
  "BALANCE_OFFSET_L3_Elite_Mental_Biased",
...
async function loadActiveTuning(client: PrismaClient) {
  ...
  if (!powerSet || !combatSet) {
    throw new Error("Missing ACTIVE Power or Combat tuning set. Elite/Boss defence validation does not seed tuning.");
  }
...
function exactCommand() {
  return ["npx", "--yes", "tsx", "scripts/combatLab.eliteBossDefenceValidation.ts", ...process.argv.slice(2)].join(" ");
}
...
function tierFromName(name: string): TierName {
  return name.includes("_Boss_") ? "BOSS" : "ELITE";
}

function verdictFor(value: number | null, tier: TierName, defeatedCount: number, censoredCount: number): Verdict {
  if (defeatedCount <= 0 || value === null) return "UNCLEAR";
  if (censoredCount > 0 && censoredCount >= defeatedCount) return "HIGH/CENSORED";
  if (tier === "ELITE") {
    if (value < 4) return "LOW";
    if (value > 6) return censoredCount > 0 ? "HIGH/CENSORED" : "HIGH";
    return "PASS";
  }
...
function scenario(attack: AttackProfile, target: TargetProfile, options: CliOptions): CombatScenario {
  return {
    name: `L3 Elite/Boss Defence ${attack.channel} ${ATTACK_NAME} vs ${target.name}`,
...
function rowFromReport(attack: AttackProfile, target: TargetProfile, report: CombatSuiteReport): ResultRow {
  ...
  return {
    ...
    censoredCount,
    verdict: verdictFor(avg, target.tier, metrics.sampleCount, censoredCount),
  };
}
...
async function discoverAssets() {
  ...
  const rows = await client.monster.findMany({
    where: {
      ...
      source: "CAMPAIGN",
      isReadOnly: false,
      OR: [{ name: ATTACK_NAME }, { name: { in: TARGET_NAMES } }],
    },
    orderBy: { name: "asc" },
    include: {
      naturalAttack: true,
  ...
  const targets: TargetProfile[] = TARGET_NAMES.map((name) => {
    const entry = adapted.find((item) => item.row.name === name);
    if (!entry) throw new Error(`Missing Elite/Boss defence candidate: ${name}.`);
    return {
      name: entry.row.name,
      tier: tierFromName(entry.row.name),
...
async function buildPayload(options: CliOptions) {
  ...
  return {
    report: "Combat Lab Level 3 Elite/Boss Defence Validation",
    mode: "elite-boss-tier-dependent-defence-validation",
    ...
    doctrineNote:
      "Elite/Boss tier-dependent defence validation only. Tier-specific defence is expected; this is not final doctrine and not role asset tuning.",
...
function printPayload(payload: Awaited<ReturnType<typeof buildPayload>>) {
  ...
  console.log("\nCandidate Runtime Defence:");
  for (const target of payload.targets) {
    console.log(
      `- ${target.tier} ${target.defencePackage}: HP ${target.runtime.physicalHp}/${target.runtime.mentalHp}, dodge ${target.runtime.dodgeDice}, ` +
  ...
  console.log("\nElite/Boss Defence Matrix:");
  console.log("Tier  | Defence          | Physical                         | Mental");
  ...
  for (const tier of ["ELITE", "BOSS"] as const) {
    for (const defencePackage of ["Standard Defence", "Heavy Defence", "Physical Biased", "Mental Biased"] as const) {
      const physical = payload.rows.find((row) => row.tier === tier && row.defencePackage === defencePackage && row.channel === "physical");
      const mental = payload.rows.find((row) => row.tier === tier && row.defencePackage === defencePackage && row.channel === "mental");
      console.log(`${tier.padEnd(5)} | ${defencePackage.padEnd(16)} | ${display(physical).padEnd(32)} | ${display(mental)}`);
    }
  }
