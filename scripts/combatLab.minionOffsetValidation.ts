...
const MINION_PREFIX = "BALANCE_OFFSET_L3_Minion_HP";
const MINION_MIN = 1;
const MINION_MAX = 2;

type PrismaClient = typeof import("../prisma/client")["prisma"];
type DefencePackage = "No Defence" | "Light Defence" | "Standard Defence";
...
async function loadActiveTuning(client: PrismaClient) {
  ...
  if (!powerSet || !combatSet) {
    throw new Error("Missing ACTIVE Power or Combat tuning set. Minion offset validation does not seed tuning.");
  }
...
function exactCommand() {
  return ["npx", "--yes", "tsx", "scripts/combatLab.minionOffsetValidation.ts", ...process.argv.slice(2)].join(" ");
}
...
function packageFromName(name: string): DefencePackage {
  if (name.endsWith("_No_Defence")) return "No Defence";
  if (name.endsWith("_Light_Defence")) return "Light Defence";
  return "Standard Defence";
}

function hpFromName(name: string): number {
  const match = name.match(/_HP(\d+)_/);
...
function verdictFor(value: number | null, defeatedCount: number, censoredCount: number): Verdict {
  ...
  if (value < MINION_MIN) return "LOW";
  if (value > MINION_MAX) return censoredCount > 0 ? "HIGH/CENSORED" : "HIGH";
  return "PASS";
}

function scenario(attack: AttackProfile, target: TargetProfile, options: CliOptions): CombatScenario {
  return {
    name: `L3 Minion Offset ${attack.channel} ${ATTACK_NAME} vs ${target.name}`,
...
async function discoverAssets() {
  ...
  const rows = await client.monster.findMany({
    where: {
      ...
      source: "CAMPAIGN",
      isReadOnly: false,
      OR: [{ name: ATTACK_NAME }, { name: { startsWith: MINION_PREFIX } }],
    },
    orderBy: { name: "asc" },
    include: {
      naturalAttack: true,
  ...
  const targets: TargetProfile[] = adapted
    .filter((entry) => entry.row.name.startsWith(MINION_PREFIX))
    .map((entry) => ({
      name: entry.row.name,
      hp: hpFromName(entry.row.name),
    ...
    .sort((left, right) => left.hp - right.hp || left.defencePackage.localeCompare(right.defencePackage));
  return {
    campaignId: campaign.id,
...
async function buildPayload(options: CliOptions) {
  ...
  for (const attack of discovery.attacks) {
    for (const target of discovery.targets) rows.push(rowFromReport(attack, target, runScenarioSuite(scenario(attack, target, options))));
  }
  return {
    report: "Combat Lab Level 3 Minion Offset Validation",
    mode: "minion-offset-validation",
    ...
    doctrineNote:
      "Minion offset validation only. Tier-dependent defence is expected; Minion results should not reduce Soldier/Elite/Boss defence assumptions. This is not final doctrine and not role asset tuning.",
...
function printPayload(payload: Awaited<ReturnType<typeof buildPayload>>) {
  ...
  console.log("\nMinion Candidate Runtime Defence:");
  for (const target of payload.targets) {
    console.log(
      `- HP${target.hp} ${target.defencePackage}: dodge ${target.runtime.dodgeDice}, ` +
  ...
  console.log("\nMinion Matrix (1-2 ATK band):");
  console.log("HP | Defence          | Physical                         | Mental");
  console.log("-".repeat(86));
  for (const hp of [4, 5, 6]) {
    for (const defencePackage of ["No Defence", "Light Defence", "Standard Defence"] as const) {
      const physical = payload.rows.find((row) => row.hp === hp && row.defencePackage === defencePackage && row.channel === "physical");
      const mental = payload.rows.find((row) => row.hp === hp && row.defencePackage === defencePackage && row.channel === "mental");
      console.log(`${String(hp).padEnd(2)} | ${defencePackage.padEnd(16)} | ${display(physical).padEnd(32)} | ${display(mental)}`);
    }
  }
