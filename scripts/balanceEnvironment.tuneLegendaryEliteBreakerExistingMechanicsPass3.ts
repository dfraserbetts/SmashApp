...
const SPEC = {
  name: ASSET_NAME,
  roleIntent:
    "Existing-mechanics anti-tank Legendary Elite 1v1 duel benchmark. Tests Intellect-resisted Control with linked mental damage only.",
  physicalHp: 30,
  mentalHp: 26,
  ...
  attack: {
    name: "BALANCE_Legendary Elite Breaker Controller Lite Role Strike",
    rawStrength: 2,
    displayedWoundsPerSuccess: 4,
    damageType: { name: "Crushing", mode: "PHYSICAL" as const },
  },
  controlPackage: {
    name: "BALANCE_Legendary Elite Breaker Controller Lite Mind Spike",
    diceCount: 3,
    potency: 1,
    cooldownTurns: 2,
...
function customNotes() {
  return [
    "BALANCE_STATUS: Level 3 Legendary Elite existing-mechanics anti-tank controller-lite benchmark asset",
    "BALANCE_SOURCE: Balance Environment Legendary Elite anti-tank existing-mechanics pass 3",
    "BALANCE_PHASE: BAL-EXISTING-MECHANICS-ANTITANK-003",
    `BALANCE_ROLE_INTENT: ${SPEC.roleIntent}`,
    "BALANCE_MECHANIC_NOTE: Uses only supported Combat Lab mechanics: natural physical pressure plus legal Control primary and linked mental damage rider.",
    "BALANCE_PACKET_NOTE: Legal structure is Control primary plus Attack secondary. No secondary Debuff packet is authored.",
...
async function findLite(prisma: PrismaClient) {
...
async function ensureLitePower(tx: Prisma.TransactionClient, monsterId: string) {
  const power = SPEC.controlPackage;
  await tx.power.create({
    data: {
      ...
      name: power.name,
      description:
        "Existing-mechanics control-lite test package. Primary Control is resisted by Intellect; linked secondary mental damage rides only when the primary succeeds.",
...
async function main() {
  ...
  try {
    ...
    const before = await findLite(prisma);
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await ensureLitePower(tx, monster.id);

      return monster;
    });

    const after = await findLite(prisma);
    console.log("Balance Environment Legendary Elite existing-mechanics anti-tank Controller Lite variant ensured.");
    console.log(`campaignId: ${BALANCE_CAMPAIGN_ID}`);
    console.log(`campaignName: ${BALANCE_CAMPAIGN_NAME}`);
    console.log(`asset: ${ASSET_NAME}`);
    console.log("DB mutation: scoped upsert for BALANCE_Legendary Elite Breaker Controller Lite only.");
    console.log("Existing mechanics only: natural physical pressure plus legal Control primary and linked mental damage rider.");
    console.log("No secondary Debuff packet is authored.");
    ...
    console.log("No Breaker, Breaker Debuffer, Breaker Controller, Duelist, Hexer, Stoneguard, Hawkshot, Ranger, Arcane Sage, Dragon/Lich, Minion/Soldier/Elite/Boss role, calibration, runtime, formula, scalar, tuning, UI, or docs values were changed by this script.");
    console.log(JSON.stringify(
      {
        ...
        controlPackage: {
          name: SPEC.controlPackage.name,
          primaryPacket: {
            intention: "CONTROL",
            dice: `${SPEC.controlPackage.diceCount}x${SPEC.attackDie}`,
            resistedBy: SPEC.controlPackage.resistAttribute,
            effect: "Force No Main Action",
            potency: SPEC.controlPackage.potency,
            durationTurns: SPEC.controlPackage.controlDurationTurns,
            cooldownTurns: SPEC.controlPackage.cooldownTurns,
          },
          secondaryPackets: [
            {
              intention: "ATTACK",
              ...
              channel: "mental",
              woundsPerPrimarySuccess: SPEC.controlPackage.damagePotency,
              damageType: SPEC.controlPackage.damageType,
            },
          ],
        },
        before,
