...
export function successCountForRoll(roll: number): number {
  if (roll >= 11) return 3;
  if (roll >= 8) return 2;
  if (roll >= 4) return 1;
  return 0;
...
export function rollDie(die: CombatDieSize, rng: Rng): { roll: number; successes: number } {
  const roll = Math.floor(rng() * diceSides(die)) + 1;
  return { roll, successes: successCountForRoll(roll) };
}

export function rollDice(count: number, die: CombatDieSize, rng: Rng) {
  const rolls = Array.from({ length: Math.max(0, Math.trunc(count)) }, () => rollDie(die, rng));
  return {
    rolls,
