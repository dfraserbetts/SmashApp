...
export async function GET() {
  try {
    ...
    const rows = await prisma.wardingOption.findMany({
      orderBy: { name: "asc" },
...
export async function POST(req: Request) {
  try {
    ...
    const created = await prisma.wardingOption.create({
      data: { name },
      select: { id: true, name: true },
    });
...
export async function PATCH(req: Request) {
  try {
    ...
    const updated = await prisma.wardingOption.update({
      where: { id },
      data: { name },
      select: { id: true, name: true },
    });
...
export async function DELETE(req: Request) {
  try {
    ...
    await prisma.wardingOption.delete({ where: { id } });
