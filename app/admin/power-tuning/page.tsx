...
import {
  POWER_TUNING_ADMIN_GROUPS,
  POWER_TUNING_ADMIN_METADATA,
  type PowerTuningAdminGroup,
  type PowerTuningValueFormat,
} from "@/lib/config/powerTuningAdminMetadata";
import {
...
function getStatusChipClass(status: PowerTuningConfigStatus): string {
...
function toDraftInputs(values: Record<string, number>): Record<string, string> {
  ...
  for (const key of POWER_TUNING_CONFIG_KEY_ORDER) {
    next[key] = String(values[key] ?? DEFAULT_POWER_TUNING_VALUES[key] ?? 0);
  }
  return next;
}

function getSnapshotValue(snapshot: PowerTuningSnapshot | null, key: string): number {
  return snapshot?.values[key] ?? DEFAULT_POWER_TUNING_VALUES[key] ?? 0;
}
...
function formatHint(format?: PowerTuningValueFormat): string | null {
  if (format === "share") return "0.00 to 1.00 typical";
  if (format === "multiplier") return "1.00 = neutral";
  return null;
}

function formatAffects(value: string): string {
  if (value === "cost_and_axis") return "Cost + Axis";
  return value === "axis" ? "Axis" : "Cost";
}
...
async function fetchAdminPowerTuning(setId?: string | null): Promise<AdminPowerTuningResponse> {
  ...
  if (!response.ok || !payload?.selectedSet) {
    throw new Error(payload?.error ?? "Failed to load power tuning");
  }
  return payload;
}

export default function AdminPowerTuningPage() {
  const [data, setData] = useState<AdminPowerTuningResponse | null>(null);
  const [activeSnapshot, setActiveSnapshot] = useState<PowerTuningSnapshot | null>(null);
  const [selectedSetId, setSelectedSetId] = useState<string | null>(null);
  const [draftValues, setDraftValues] = useState<Record<string, string>>(
    toDraftInputs(DEFAULT_POWER_TUNING_VALUES),
  );
  ...
  const dirtyKeys = useMemo(
    () =>
      isDraft
        ? POWER_TUNING_CONFIG_KEY_ORDER.filter((key) =>
  ...
  const groupedKeys = useMemo(
    () =>
      POWER_TUNING_ADMIN_GROUPS.map((group) => {
        const allKeys = POWER_TUNING_CONFIG_KEY_ORDER.filter(
          (key) => POWER_TUNING_ADMIN_METADATA[key]?.group === group,
        );
        const normalizedSearch = searchQuery.trim().toLowerCase();
        const keys = allKeys.filter((key) => {
          if (!normalizedSearch) return true;
          const metadata = POWER_TUNING_ADMIN_METADATA[key];
  ...
  const load = useCallback(
    async (nextSetId?: string | null) => {
      if (isDirty && !window.confirm("Discard unsaved power tuning edits?")) return;
      setLoading(true);
      setError(null);

      try {
        const payload = await fetchAdminPowerTuning(nextSetId ?? selectedSetId);
        const nextActiveSnapshot =
          payload.activeSetId === payload.selectedSet.setId
            ? payload.selectedSet
            : (await fetchAdminPowerTuning(payload.activeSetId)).selectedSet;
        ...
        setError(String((loadError as { message?: unknown })?.message ?? "Failed to load power tuning"));
      } finally {
        setLoading(false);
      }
    },
  ...
  async function createDraft() {
    ...
    try {
      const response = await fetch("/api/admin/power-tuning", {
        method: "POST",
  ...
  function resetSectionToActive(group: PowerTuningAdminGroup) {
    setDraftValues((current) => {
      const next = { ...current };
      for (const key of POWER_TUNING_CONFIG_KEY_ORDER) {
        if (POWER_TUNING_ADMIN_METADATA[key]?.group === group) {
  ...
  function resetAllToActive() {
    setDraftValues(toDraftInputs(activeSnapshot?.values ?? DEFAULT_POWER_TUNING_VALUES));
  }
    ...
    <div className="space-y-6">
      ...
      <section className="rounded-lg border border-zinc-800 p-4">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          <div>
            <h2 className="text-lg font-medium">Phase 6 Power Tuning</h2>
            <p className="mt-1 text-sm text-zinc-400">
              Versioned config sets for Summoning Circle power tuning. Combat Tuning is unchanged.
            </p>
            <p className="mt-2 text-xs text-zinc-500">
              Exposing {POWER_TUNING_CONFIG_KEY_ORDER.length} current tuning keys across{" "}
              {POWER_TUNING_ADMIN_GROUPS.length} sections.
            </p>
          </div>
          <span
            className={[
      ...
      <section className="rounded-lg border border-zinc-800 p-4 space-y-4">
        <div>
          ...
          <p className="mt-1 text-xs text-zinc-500">
            Drafts clone the current active values. Notes are display-only after creation in the current API shape.
          </p>
        </div>
      ...
      {selectedSet ? (
        <section className="rounded-lg border border-zinc-800 p-4 space-y-5">
              ...
              {isDraft ? (
                ...
                <>
                  <button
                    ...
                    className="rounded border border-zinc-700 px-3 py-2 text-sm hover:bg-zinc-900 disabled:opacity-60"
                  >
                    {saving ? "Working..." : "Unarchive As Draft"}
                  </button>
          ...
          <div className="grid gap-3 text-xs sm:grid-cols-4">
            <div className="rounded border border-zinc-800 bg-zinc-950/40 p-3">
              ...
              <div className="mt-1 text-lg text-zinc-100">{POWER_TUNING_CONFIG_KEY_ORDER.length}</div>
            </div>
            <div className="rounded border border-zinc-800 bg-zinc-950/40 p-3">
          ...
          <div className="space-y-4">
            {groupedKeys.map(({ group, keys, totalChangedCount }) => {
              ...
              return (
                <div key={group} className="rounded border border-zinc-800 p-4 space-y-3">
                    ...
                    {keys.map((key) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p className="mt-2 text-xs text-zinc-400">{metadata?.description ?? "Power tuning value."}</p>
                          ...
                          <div className="mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-zinc-500">
                            ...
                            {metadata?.suggestedMin != null || metadata?.suggestedMax != null ? (
                              <span>
                                Suggested range: {metadata.suggestedMin ?? "?"} to {metadata.suggestedMax ?? "open"}
                              </span>
                            ) : null}
